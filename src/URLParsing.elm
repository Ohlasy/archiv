...
encodeHashString d =
    let
        encodePair (key, val) = (encodeUri key) ++ "=" ++ (encodeUri val)
        encodedPairs = List.map encodePair (Dict.toList d)
        encodedHash = String.join "&" encodedPairs
    in
        "#" ++ encodedHash
...
decodeHashString startsWithHashMarkThenParams =
    case (String.uncons startsWithHashMarkThenParams) of
        Just ('#', rest) -> Just (decodeParams rest)
        Nothing -> Nothing
        _ -> Nothing
...
decodeParams stringWithAmpersands =
    let
        ...
        decodedPairs = List.filterMap decodePair eachPair
    in
